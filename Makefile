...
$(WINDOWS):
	GOOS=windows GOARCH=amd64 go build -v -o ./dist/$(VERSION)/$(WINDOWS) -ldflags="-s -w -X main.version=$(VERSION)" ./main.go
...
$(LINUX):
	GOOS=linux GOARCH=amd64 go build -v -o ./dist/$(VERSION)/$(LINUX) -ldflags="-s -w -X main.version=$(VERSION)" ./main.go
...
$(DARWIN_AMD):
	GOOS=darwin GOARCH=amd64 go build -v -o ./dist/$(VERSION)/$(DARWIN_AMD) -ldflags="-s -w -X main.version=$(VERSION)" ./main.go
...
$(DARWIN_ARM):
	GOOS=darwin GOARCH=arm64 go build -v -o ./dist/$(VERSION)/$(DARWIN_ARM) -ldflags="-s -w -X main.version=$(VERSION)" ./main.go
...
## Build everything
build: sha256sums windows linux darwin-amd darwin-arm reset-env
